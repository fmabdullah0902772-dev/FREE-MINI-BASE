...
const NEWSLETTER_JID = "120363423033116549@newsletter";
const NEWSLETTER_NAME = "FM ABDULLAH";
const BOT = "ABDULLAH-MD";

const buildCaption = (video) => {
  const duration = video.timestamp || video.duration || "N/A";

  return (
    `*🎬 FM ABDULLAH VIDEO PLAYER*\n\n` +
    `╭───────────────◆\n` +
    `│ 📑 Title: ${video.title}\n` +
    `│ ⏳ Duration: ${duration}\n` +
    `╰────────────────◆\n\n` +
...
const getContextInfo = (query = "", video = {}) => ({
  ...
  externalAdReply: {
    ...
    sourceUrl: video.url || "https://whatsapp.com/channel/0029VbBwUOZ6BIEaxmYl3m1Z"
  },
